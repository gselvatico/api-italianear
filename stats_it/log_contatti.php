...
if($num>0){
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        $stats_it_item=array(       
			...
			"nome_attiivita" => $nome_attiivita,
            "localita" => $localita,
            "ultimo"=>date("d/m/Y", strtotime($ultimo))
        );
    ...
    if (count($records) === 0) {
        ...
        echo '<h2>Statistiche dei contatti visitati dal '. date("d/m/Y", strtotime($date_min)).' al '.date("d/m/Y", strtotime($date_max)).' </h2>';
        // Genera la tabella HTML
        echo '<table style: font-family: Arial, sans-serif;
        font-size: 8px; >';
        echo '<thead><tr><th>n.</th><th>Nome</th><th>Attività</th><th>Località</th><th>Ultima data</th></tr></thead>';
        echo '<tbody>';
        foreach ($records as $record) {
            echo '<tr>';
            echo '<td>' . htmlspecialchars($record['nUtenti']) . '</td>';
            echo '<td>' . htmlspecialchars($record['nome_contatto']) . '</td>';
            echo '<td>' . htmlspecialchars($record['nome_attiivita']) . '</td>';
            echo '<td>' . htmlspecialchars($record['localita']) . '</td>';
            echo '<td>' . htmlspecialchars($record['ultimo']) . '</td>';
            echo '</tr>';
        }
        // ******i totali**********
